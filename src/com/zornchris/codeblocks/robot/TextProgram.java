...
public class TextProgram extends Program {
    ...
    public TextProgram(Plugin plugin, Block start, String fileName, Challenge challenge){
    ...
    private void loadProgram() {
        ...
        try {
            Scanner scn = new Scanner(new File("plugins/CodeBlocks/TextPrograms/" + fileName + ".txt"));
    ...
    private void processLines() {
        ...
        for(String line : lines) {
            ...
            if(line.length() > 0) {
                ...
                // Find Branches
                    ...
                    if(line.indexOf("Sense") > -1) {
                        ...
                        if(startParen > -1 && endParen > -1) {
                            String blockToCheck = line.substring(startParen + 1, endParen);
                            current = new BranchingBlock(BlockType.Sense, blockToCheck);
                            branches.add((BranchingBlock) current);
                            //System.out.println("creating branch " + blockToCheck);
                        }
                    }
                }
                    ...
                    current = new FunctionBlock(BlockType.FunctionCall, funcName, null);
                }
                else if(line.indexOf(":") > -1) {
                    String funcName = line.substring(0, line.indexOf(":"));
                    System.out.println("Function Def " + funcName);
                    current = new FunctionBlock(BlockType.FunctionDef, null, funcName);
                ...
                if(current instanceof BranchingBlock && (((BranchingBlock) current).ifBlock != null) && (((BranchingBlock) current).elseBlock == null)) {
                    previous.next = null;
                }
                ...
                /*else if(previous.type == BlockType.FunctionStart)
                    previous.next = null;
                else if(line.equalsIgnoreCase("fi"))
                    previous.next = null;
                */
                previous = current;         // We're done with current, so move it to previous
            }
        }
    }
    ...
    {
        System.out.println("Evaluating " + b.type);
        ...
        switch(b.type) {
            ...
            case Sense:
                boolean conditionValue = robot.senseBlockInFront(((BranchingBlock) b).param);
                System.out.println("Condition " + conditionValue);
                programCounter = (conditionValue) ? ((BranchingBlock) b).ifBlock : ((BranchingBlock) b).elseBlock;
        }
    ...
    private class CodeBlock {
        ...
        @Override
        public String toString() { return "(" + type + " " + next + ")"; }
    }
    ...
    private class BranchingBlock extends CodeBlock {
        ...
        @Override
        public String toString() { return type + " " + param + " " + ifBlock + " " + elseBlock; }
    }
}
